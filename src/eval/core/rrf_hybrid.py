...
def score_neural_batch(
    ...
    max_length: int = 512,
    truncation_strategy: str | bool = "longest_first",  # or "only_second"
    normalize: str | None = None,  # None | "sigmoid" | "zscore"
) -> List[float]:
    """
    Returns scores aligned with items. Uses raw logits by default (best for ranking).
    """
    if not items:
        return []
    ...
    out = model(**enc)                  # HF returns SequenceClassifierOutput
...
def hybrid_rrf_search(
    ...
    dense_results = retriever._query_dense(dense_vec, media_type, qf, candidate_pool_dense) # 300 dense results
    sparse_results = retriever._query_sparse(sparse_vec, media_type, qf, candidate_pool_sparse) # 20 sparse results
    ...
    base_top_ids = base_ids_ranked[:100]
    ...
    neural_sorted_points = sorted(
        dense_top30_points,
        key=lambda p: id2neural.get(int(p.id), float("-inf")),
        reverse=True
    )
    neural_ids_B = [int(p.id) for p in neural_sorted_points]    

    # --- Stage 5: Final RRF fuse the two lists (A and B), return top_k
    final_rrf = rrf_fuse([meta_ids_A, neural_ids_B], k=rrf_k)
    ...
    fused_out: List[Dict] = []
    fused_list = [v['point'] for k, v in fused_all.items()]
    for idx, pt in enumerate(fused_list[:20], start=1):
        pid = int(pt.id)
